...
An *AKMBUILD* file is used by *akms(1)* tool to build and install an out-of-tree dynamic kernel module on the user`'s system.
...
The following variables are always provided by *akms(1)* and MUST NOT be overriden.
...
The following variables are provided by *akms(1)* when running the *build* function and MUST NOT be overriden.
...
ifdef::backend-manpage[APKBUILD(5)]
//ifndef::backend-manpage[{man-uri}/akms.1.adoc[akms(1)]]
